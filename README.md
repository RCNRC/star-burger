...
Сайт работает по ссылке: [https://ilyaprojects.ru/](https://ilyaprojects.ru/). [Ссылка](https://ilyaprojects.ru/manager) на вкладку для менеджеров.

Это репозиторий сата сети ресторанов Star Burger. Здесь можно заказать превосходные бургеры с доставкой на дом.
...
Версия `nodejs` должна быть не младше `14.0` и не старше `16.16`. Лучше ставьте `16.16.0`, её мы тестировали. Версия `npm` не важна. Как обновить Node.js читайте в статье: [How to Update Node.js](https://phoenixnap.com/kb/update-node-js-version).
